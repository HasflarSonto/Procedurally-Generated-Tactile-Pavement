...
def create_polylines_from_array_with_remap(data_array):
    """
    Parses an array of strings containing coordinate data, remaps them, and creates polylines.
    ...
    """
    all_points = []  # To store all valid points for calculating bounds
    polylines = []
    ...
    for data_string in data_array:
        ...
        try:
            ...
            # Log and skip malformed data strings
            print(f"Skipping malformed data entry: {data_string} | Error: {e}")
            continue

    # Step 2: Calculate bounds
    if not all_points:
        print("No valid points found in the entire input.")
        return []
    ...
    # Step 3: Remap points and create polylines
    remapped_polylines = []
    for points in polylines:
        try:
            remapped_points = [
                (
                    remap(p[0], min_x, max_x, 0, 100),
                    remap(p[1], min_y, max_y, 0, 100),
                    remap(p[2], min_z, max_z, 0, 100),
                )
                for p in points
            ]
            ...
            print(f"Error creating polyline for points: {points} | Error: {e}")
            continue
